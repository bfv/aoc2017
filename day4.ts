let day4Input = `sayndz zfxlkl attjtww cti sokkmty brx fhh suelqbp
...
day4(day4Input);
type fEquals = (str1: string, str2: string) => boolean;

function day4(data: string) {

    let result =  0;
    let resultb = 0;
    let data1 = data.split('\n');

    for (let line of data1) {
        result += parseLine(line, equal4);
        resultb += parseLine(line, equal4b);
    }
...
function equal4(str1: string, str2: string): boolean {
    return (str1 == str2);
}
